...
class ThreeEma:
    ...
    def determine_signal(self,dframe):
        ...
        if (close.iloc[-1] < ema_50.iloc[-1]) and (ema_5.iloc[-3] > ema_20.iloc[-3]) and (
                ema_5.iloc[-2] < ema_20.iloc[-2] < ema_50.iloc[-2]):
            action = -1
        # BUY CRITERIA: 5ema has crossed over 20ema such that 5ema > 20ema > 50ema and closing price is > 50 ema
        elif (close.iloc[-1] > ema_50.iloc[-1]) and (ema_5.iloc[-3] < ema_20.iloc[-3]) and (
                ema_5.iloc[-2] > ema_20.iloc[-2] > ema_50.iloc[-2]):
            action = 1
