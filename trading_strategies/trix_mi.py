...
class TrixMI:
    ...
    # calculates the trix indicator (trix) using ta library
    def calculate_trix(self):
        self.df['trix'] = ta.trend.TRIXIndicator(close = self.close).trix() # n is left at the default of 15
    ...
    def determine_signal(self, dframe):
        ...
        # BUY SIGNAL: when mi drops below 26.5 and trix indicator was showing a downtrend, i.e. was below 0
        if dframe['mi'].iloc[-1] < 26.5 and dframe['mi'].iloc[-2] >= 26.5 and dframe['trix'].iloc[-1] < 0 :
            signal = 1
        # SELL SIGNAL: when mi drops below 26.5 and trix indicator was showing an uptrend, i.e. was above 0
        elif dframe['mi'].iloc[-1] < 26.5 and dframe['mi'].iloc[-2] >= 26.5 and dframe['trix'].iloc[-1] > 0:
            signal = -1

        return (signal, dframe['trix'].iloc[-2])
    ...
    def find_all_signals(self, plot_df):
        ...
        end = start + 44  # where the current window will stop (exclusive of the element at this index)
        ...
        plot_df.loc[plot_df.index[-1], 'signal'] = self.determine_signal(plot_df[-44:])[0]
    ...
    def plot_graph(self):
        ...
        visualisation.add_subplot(plot_df['trix'], panel=1, color="orange", ylabel="TRIX")
        visualisation.add_subplot(plot_df['zero_line'], panel=1, color="k", secondary_y=False, width=0.75, linestyle='solid')
        visualisation.add_subplot(plot_df['mi'], panel=2, color='m', width=0.75, ylabel='MI')
        visualisation.add_subplot(plot_df['mi_cross_line'], panel=2, color='k', secondary_y=False, width=0.75, linestyle='solid')

        # create final plot with title
        visualisation.plot_graph("TRIX MI Strategy")
